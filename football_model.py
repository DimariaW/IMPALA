...
def to_tensor(x, unsqueeze=None):
    if isinstance(x, (list, tuple)):
        ...
        if x.dtype in [np.int32, np.int64]:
            t = paddle.to_tensor(x, dtype="int64")
        else:
            t = paddle.to_tensor(x, dtype="float32")
...
def split_batched_tensor_and_transpose(x: Union[List, Tuple, Dict, paddle.Tensor], batch_len: int, time_len: int):
    if isinstance(x, (list, tuple)):
        return type(x)(split_batched_tensor_and_transpose(xx, batch_len, time_len) for xx in x)
    elif isinstance(x, dict):
        return type(x)((key, split_batched_tensor_and_transpose(xx, batch_len, time_len)) for key, xx in x.items())
    elif isinstance(x, paddle.Tensor):
        x = x.reshape([batch_len, time_len]+x.shape[1:])
        x = x.transpose(perm=[1, 0] + list(range(2, len(x.shape))))
        return x
...
if __name__ == "__main__":
    ...
    #z=batchify(obss, unsqueeze=0)
    z1 = batchify(obss[:30], unsqueeze=0)
    z2 = batchify(obss[30:], unsqueeze=0)
    z = batchify([z1, z2], unsqueeze=None)
    z = to_tensor(z, unsqueeze=None)
    z = split_batched_tensor_and_transpose(z, 13, 5)
    net = FootballNet()
    s = net(z)
    assert(True)
