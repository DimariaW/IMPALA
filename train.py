...
from football_actor import Actor
...
class Learner(object):
    ...
    def _reader(self):
        while True:
            ...
            self.sample_total_steps += sample_data['actions'].shape[0]
            self.batch_buffer.append(sample_data)

            buffer_size = sum(
                [data['actions'].shape[0] for data in self.batch_buffer])
            if buffer_size >= self.config['train_batch_size']:
                batch = batchify(self.batch_buffer, unsqueeze=None)
                self.batch_buffer = []
    ...
    def run_learn(self):
        ...
        for obs_np, actions_np, behaviour_logits_np, rewards_np, dones_np in self._batch_reader()():
