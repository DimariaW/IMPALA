...
import vtrace
from football_model import split_batched_tensor_and_transpose
...
class VTraceLoss(object):
    def __init__(self,
        ...
        self.vtrace_returns = vtrace.from_importance_weights(
            ...
            discounts=(1. - dones).astype('float32') * discount,
            rewards=rewards,
            values=values,
...
class IMPALA(parl.Algorithm):
    ...
    def learn(self, obs, actions, behaviour_logits, rewards, dones,
        ...
        """
        self.model.train()
        target_logits, values = self.model(obs)  # B*Action_dim
        ...
        target_actions_log_probs = self._log_prob(target_logits, actions)  # B
        behaviour_actions_log_probs = self._log_prob(behaviour_logits, actions)  # B
